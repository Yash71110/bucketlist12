...
-- -------------------------------------------------------------------------
-- 4. APP SETTINGS
-- Another singleton row (same check(id) trick) for small shared
-- preferences — currently just the relationship start date, which powers
-- the "together for X days" widget.
-- -------------------------------------------------------------------------
...
-- Atomic increment for gallery reactions — an RPC rather than a plain
-- client-side update so two rapid taps can never overwrite each other.
